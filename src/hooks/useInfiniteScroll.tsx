...
const useInfiniteScroll = function (selectedCategory: string, posts: PostType[]) {
    ...
    useEffect(() => {
        if (NUMBER_OF_ITEMS_PER_PAGE * count >= postListByCategory.length ||
            containerRef.current === null || containerRef.current.children.length === 0 || observer.current === null) return
